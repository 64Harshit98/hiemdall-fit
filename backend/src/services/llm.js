...
- INJURIES & LIMITATIONS (HIGHEST PRIORITY — this overrides goal, split_preference, liked exercises, and carry_forward): the profile's injuries field is free text describing the user's injuries, pain, and physical limitations (e.g. "leg injury", "lower back pain", "left shoulder impingement"). When it is non-empty you MUST:
    1. Identify the affected body region(s) and infer the muscle groups and movement patterns that load them. For example a "leg injury" implicates the quads, hamstrings, glutes, and calves and rules out loaded squatting, lunging, hip-hinge/deadlift, leg press, calf raise, and explosive/plyometric leg work; a "lower back" issue rules out axial-loaded squats, deadlifts/RDLs, barbell rows, and good mornings; a "shoulder" issue rules out overhead pressing and heavy horizontal pressing.
    2. NEVER prescribe any exercise — neither as a main exercise NOR as an alternate_exercise — that loads, stretches, or stresses the injured area. Do not program squats, calf raises, Romanian deadlifts, lunges, or similar movements for someone with a leg/knee/ankle injury, and apply the same logic to every other injury. When in doubt, leave the movement out.
    3. Replace the avoided work with safe alternatives that train other body parts or that isolate the injured region in a pain-free, low-load, rehab-appropriate way only if clearly safe. It is acceptable to reduce or skip direct work for the injured area entirely and shift volume to unaffected body parts.
    4. The week_summary MUST explicitly name the injury/limitation it accounted for and list which movements it deliberately avoided and what it substituted instead. If injuries is empty, state "no injuries reported".
...
- carry_forward, when provided, is a list of exercises (with sets/reps) the user did NOT finish last week. You OWN the whole week: schedule EVERY carry_forward exercise into this week's training days yourself, placing each on a day whose focus matches its muscle group (e.g. a carried squat variation goes on a lower/legs day, a carried row on a pull/back day). Treat them as additional required volume on top of your normal programming for those days. Critical constraints: (1) still output a balanced week with the correct number of training days AND rest days per the days_per_week range above — do NOT turn rest days into training days or drop rest to fit carried work; instead add the carried exercises to existing training days. (2) NEVER list the same exercise twice in one day — if a carried exercise duplicates one you were already going to program that day, merge them into a single entry (use the higher set count) rather than repeating it. (3) The week_summary MUST note which carried-over exercises were folded in and onto which days.`;
...
- INJURIES & LIMITATIONS (HIGHEST PRIORITY): the profile's injuries field is free text describing injuries, pain, and limitations. When non-empty, infer the affected body region and the movement patterns that load it, and NEVER prescribe a movement (main OR alternate) that loads, stretches into pain, or stresses the injured area — e.g. for a leg/knee/ankle injury avoid deep squats, lunges, calf raises, and weight-bearing leg drills. Favour gentle, pain-free mobility for unaffected areas, and only address the injured region with very light, clearly safe range-of-motion work.
